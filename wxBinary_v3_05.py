...
class dataRetrieval(masterProcessingPanel):
    ...
    def read_db(self, event):
        ...
        X=recordsAll.iloc[:,range(9,38)]
        Y=recordsAll.iloc[:,range(38,67)]
        records=records.convert_dtypes()
        del recordsAll
        X=X.convert_dtypes()
        Y=Y.convert_dtypes()
...
######################################################################
        ...
        if self.rawDataCheckBox.GetValue():
            ...
            if self.largePointsCheckBox.GetValue():
                marker = 'o'
                markersize=1.5
            self.linera, = self.velocityGraph.axes.plot(xdata1.to_numpy(), ydata1ra.to_numpy(), color=c, marker=marker, linestyle='none', linewidth=0, markersize=markersize)
            self.velocityGraph.draw(self.linera, xdata1.to_numpy(), ydata1ra.to_numpy(), True, [] )
            self.linedec, = self.velocityGraph.axes.plot(xdata1.to_numpy(), ydata1dec.to_numpy(), color=c, marker=marker, linestyle='none', linewidth=0, markersize=markersize)
