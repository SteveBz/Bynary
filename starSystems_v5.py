...
class binaryStarSystems():
    ...
    def __init__(self, numberStars, Mass_Correction, Spher_Corr=True):
        self.binaryList={}
    ...
    def addSystem(self, row, ccdm, rfactor=0):
        rowCopy = row.copy()
        R=0             # Separation in plane of sky
        ...
        v_s_corr=0      # Total spherical correction
        ...
        if str(ccdm) in self.binaryList.keys():
            ...
            (R, Rerr, V, Verr, M, v_s_corr) =binary.binaryStarSystem(rowCopy, str(ccdm))
            BIN=1
        else:
            BIN=0
            #unary = starSystem(self.star_rows[idx], rfactor=0)
            unary = starSystem(rowCopy, self.Mass_Correction, self.Spher_Corr, rfactor=0)
            self.binaryList[str(ccdm)]=unary
        
        return (ccdm, R, Rerr, V, Verr, M, BIN, v_s_corr)
...
class starSystem():
    ...
    def __init__(self, row, mass_correction, spher_corr=True, rfactor=0):
    ...
    def binaryStarSystem(self, row, ccdm):
        ...
        return (self.R, self.Rerr, self.V, self.Verr, self.M, self.v_s_corr)
    ...
    def calcV(self):
        ...
        print("spher_corr flag = ", self.spher_corr, "spher_corr MuAlphaS = ", MuAlphaS, "spher_corr MuDeltaS = ", MuDeltaS, "RV_1 = ", self.primary.radial_velocity)
        if self.spher_corr == False:
            MuAlphaS=0
            MuDeltaS=0
        ...
        v_delta_=VelDec
        self.v_s_corr=math.sqrt(float(const474*(MuAlphaS)/meanParallax)**2+(float(const474*(MuDeltaS)/meanParallax))**2)
