...
class dataRetrieval(masterProcessingPanel):
    ...
    def __init__(self, parent, mainPanel):
        ...
        self.listctrl = wx.ListCtrl(self, wx.ID_ANY, wx.DefaultPosition, wx.Size(1210,ctrl_height), wx.LC_HRULES | wx.LC_REPORT | wx.SIMPLE_BORDER | wx.VSCROLL | wx.LC_SORT_ASCENDING)
        ...
        self.listctrl.InsertColumn(10, u"Distance", wx.LIST_FORMAT_RIGHT, width=100)
        #
        self.restoreListCtrl()
        ...
        #Restore catalogue from a directory
    ...
    def read_db(self, event):
        ...
        for index, row  in records.iterrows():
            ...
            try:
                star = self.parent.starSystemList.binaryList[str(index+1)].primary
                self.parent.X[index] = {
                    'source_id': int(primaryPointer.source_id),
                    'ra': float(star.RA_),
                    'dec': float(star.DEC_),
                    'BminusR': XBminusR,
                    'mag': float(XPHOT_G_MEAN_MAG - 5 * math.log10(star.DIST / 10)),
                    'PMRA': float(star.pmra),
                    'PMRA_ERROR': float(star.pmra_error),
                    'PMDEC': float(star.pmdec),
                    'PMDEC_ERROR': float(star.pmdec_error),
                    'PARALLAX': float(star.parallax),
                    'parallax_error': float(star.parallax_error),
                    'DIST': float(star.DIST),
                    'DIST_ERR': float(star.DIST_ERR),
                    'RUWE': XRUWE,
                ...
                print(f'Error: {e}')
                print('Error Details:')
                print(f'ra: {float(star.RA_)}')
                print(f'dec: {float(star.DEC_)}')
                print(f'BminusR: {XBminusR}')
                print(f'mag: {float(XPHOT_G_MEAN_MAG - 5 * math.log10(star.DIST / 10))}')
                print(f'PMRA: {float(star.pmra)}')
                print(f'PMRA_ERROR: {float(star.pmra_error)}')
                print(f'PMDEC: {float(star.pmdec)}')
                print(f'PMDEC_ERROR: {float(star.pmdec_error)}')
                print(f'PARALLAX: {float(star.parallax)}')
                print(f'parallax_error: {float(star.parallax_error)}')
                print(f'DIST: {float(star.DIST)}')
                print(f'RUWE: {XRUWE}')
                print(f'mass_calc: {Xmass_calc}')
                print(f'mass_flame: {Xmass_flame}')
                print(f'mass_flame_upper: {Xmass_flame_upper}')
                print(f'mass_flame_lower: {Xmass_flame_lower}')
                print(f'gal_l: {Xgal_l}')
                print(f'gal_b: {Xgal_b}')
                print(f'age_flame: {Xage_flame}')
                print(f'age_flame_upper: {Xage_flame_upper}')
                print(f'age_flame_lower: {Xage_flame_lower}')
                print(f'classprob_dsc_specmod_binarystar: {Xclassprob_dsc_specmod_binarystar}')
                print(f'parallax_pmra_corr: {Xparallax_pmra_corr}')
                print(f'parallax_pmdec_corr: {Xparallax_pmdec_corr}')
                print(f'ra_parallax_corr: {Xra_parallax_corr}')
                print(f'dec_parallax_corr: {Xdec_parallax_corr}')
                print(f'ra_pmra_corr: {Xra_pmra_corr}')
                print(f'ra_pmdec_corr: {Xra_pmdec_corr}')
                print(f'dec_pmra_corr: {Xdec_pmra_corr}')
                print(f'dec_pmdec_corr: {Xdec_pmdec_corr}')
                print(f'pmra_pmdec_corr: {Xpmra_pmdec_corr}')
                print(f'phot_bp_mean_flux: {Xphot_bp_mean_flux}')
                print(f'phot_rp_mean_flux: {Xphot_rp_mean_flux}')
                print(f'phot_bp_mean_flux_error: {Xphot_bp_mean_flux_error}')
                print(f'phot_rp_mean_flux_error: {Xphot_rp_mean_flux_error}')
...
class dataFilter(masterProcessingPanel):
    ...
    def __init__(self, parent, mainPanel):
        ...
        self.static_ruwe = StaticText(self, label='|b|') 
        fgsizer.Add(self.static_ruwe, 0, wx.ALIGN_CENTER_HORIZONTAL|wx.ALL, 5)
...
# Plot position on sky for chosen binaries.
    ...
    def OnPlot(self, event=0):
        ...
        if self.showGalacticCoordsCheckBox.GetValue():
            self.skyGraph.axes.set_ylabel("Galactic 'b' (deg)", fontsize=FONTSIZE)
            self.skyGraph.axes.set_xlabel("Galactic 'l' (deg)", fontsize=FONTSIZE)
        else:
...
######################################################################
        ...
        self.velocityGraph.axes.set_title("<n> binary pairs showing actual velocity and Newtonian expectation", fontsize=FONTSIZE)
        v2D_err
        if self.combo_xy_option.GetSelection() == 0:
            ...
            self.velocityGraph.axes.set_title("<n>Binary pairs showing actual 2D velocity and Newtonian expectation against separartion", fontsize=FONTSIZE)
        elif self.combo_xy_option.GetSelection() == 1:
            xdata1 = self.parent.binaryDetail.r * self.parent.status['include']
            ...
            self.velocityGraph.axes.set_ylabel(r'$\tilde{v}$ in sky plane', fontsize=FONTSIZE)
            self.velocityGraph.axes.set_xlabel(r'2D separation, $r_{sky}$ [$pc$]', fontsize=FONTSIZE)
            self.velocityGraph.axes.set_title("<n>Binary pairs showing 2D velocity over Newtonian expectation against separartion", fontsize=FONTSIZE)
        elif self.combo_xy_option.GetSelection() == 2:
            ...
            self.velocityGraph.axes.set_ylabel(r'$\tilde{v}$ in sky plane', fontsize=FONTSIZE)
            self.velocityGraph.axes.set_xlabel(r'$r_{sky}$ / $r_{MOND}$', fontsize=FONTSIZE)
            self.velocityGraph.axes.set_title("<n>Binary pairs showing 2D velocity over Newtonian expectation against separartion by MOND radius", fontsize=FONTSIZE)
            # Remove the secondary x-axis if it already exists
            try:
                if hasattr(self, 'twinx'):
                    self.twinx.remove()
            ...
            self.velocityGraph.axes.set_ylabel(r'2D relative velocity in sky plane [$km s^{-1}$]', fontsize=FONTSIZE)
            self.velocityGraph.axes.set_xlabel(r'$r_{sky}$ / $r_{MOND}$', fontsize=FONTSIZE)
            self.velocityGraph.axes.set_title("<n>Binary pairs showing actual 2D velocity and Newtonian expectation against separartion by MOND radius", fontsize=FONTSIZE)
            # Remove the secondary x-axis if it already exists
            try:
                if hasattr(self, 'twinx'):
                    self.twinx.remove()
