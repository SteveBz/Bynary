...
######################################################################
        ...
        self.MassPlot.axes.set_xlabel('Calulated binary mass ($M_{\odot}$)', fontsize=FONTSIZE)
        self.MassPlot.axes.set_ylabel('FLAME spectrometer mass from Gaia ($M_{\odot}$)', fontsize=FONTSIZE)
        ####################################################################################################
        self.parent.binaryDetail['include']=self.parent.status['include']
        M=self.parent.binaryDetail.loc[(self.parent.binaryDetail['include'] > 0)]
        ...
        if prntVersion:
            ...
            self.MassPlot.axes.set_title(f"{ROWCOUNTMATRIX['BIN']:,} binary stars, Gaia {RELEASE}, Gaia FLAME spectrometer mass vs calculated mass", fontsize=FONTSIZE)
